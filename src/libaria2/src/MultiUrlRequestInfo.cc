...
int MultiUrlRequestInfo::prepare()
{
    ...
    if (!option_->blank(PREF_LOAD_COOKIES)) {
      ...
      if (cookieFile.isFile() &&
        ...
        if (cookieFile.getExtname().compare("tmp") == 0) {
            A2_LOG_INFO(
                fmt("The cookie is a temporary file, we will delete it"));
            cookieFile.remove();
        }
      }
      else {
  ...
  #ifdef HAVE_OPENSSL
      if (util::getSslca(cafile)) {
        if (!clTlsContext->addTrustedCACertFile(cafile)) {
          A2_LOG_INFO(MSG_WARN_NO_CA_CERT);
        }
      }
      else
...
error_code::Value MultiUrlRequestInfo::getResult()
{
  ...
  if (!option_->blank(PREF_SAVE_SESSION)) {
    ...
    if (sessionSerializer.save(savename)) {
      ...
      A2_LOG_NOTICE(
          fmt(_("Failed to serialize session to '%s'."), savename.c_str()));
    }
  }
  SingletonHolder<Notifier>::clear();
  return returnValue;
