...
{
  std::shared_ptr<Option> option = getOption();
  ...
  DownloadEngine* e = getDownloadEngine();
  const std::string pcook = option->get(PREF_LOAD_COOKIES);
  if (option->getAsBool(PREF_SELECT_LEAST_USED_HOST)) {
    e->getRequestGroupMan()->getUsedHosts(usedHosts);
  }
  // by adonais
  if (!pcook.empty()) {
    File cookieFile(pcook);
    if (cookieFile.isFile() && e->getCookieStorage()->load(cookieFile.getPath(), Time().getTimeFromEpoch())) {
      A2_LOG_NOTICE(fmt("executeInternal: we load-cookies setting"));
      if (cookieFile.getExtname().compare("tmp") == 0) {
          cookieFile.remove();
          A2_LOG_INFO(fmt("executeInternal: The cookie is a temporary file, we will delete it"));
      }
    }
  }
  setRequest(
      getFileEntry()->getRequest(getRequestGroup()->getURISelector().get(),
                                 option->getAsBool(PREF_REUSE_URI),
                                 usedHosts, option->get(PREF_REFERER),
                                 // Don't use HEAD request when file
                                 // size is known.
                                 // Use HEAD for dry-run mode.
                                 (getFileEntry()->getLength() == 0 &&
                                  option->getAsBool(PREF_USE_HEAD)) ||
                                       option->getAsBool(PREF_DRY_RUN)
                                     ? Request::METHOD_HEAD
                                     : Request::METHOD_GET));
  if (!getRequest()) {
    if (getSegmentMan()) {
