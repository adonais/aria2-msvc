...
#  endif // !__ANDROID__ && !ANDROID
  ...
  {
    OptionHandler* op(new BooleanOptionHandler(
        PREF_ENABLE_NGWEB, TEXT_ENABLE_NGWEB, A2_V_FALSE, OptionHandler::OPT_ARG));
    op->addTag(TAG_NGWEB);
    handlers.push_back(op);
  }
  {
  ...
  {
    OptionHandler* op(new UnitNumberOptionHandler(
        PREF_MIN_SPLIT_SIZE, TEXT_MIN_SPLIT_SIZE, "1M", 2_k, 1_g, 'k'));
    op->addTag(TAG_BASIC);
    op->addTag(TAG_FTP);
    op->addTag(TAG_HTTP);
    op->setInitialOption(true);
  ...
  {
    OptionHandler* op(new UnitNumberOptionHandler(
        PREF_PIECE_LENGTH, TEXT_PIECE_LENGTH, "1M", 2_k, 1_g));
    op->addTag(TAG_ADVANCED);
    op->addTag(TAG_FTP);
    op->addTag(TAG_HTTP);
    op->setInitialOption(true);
  ...
  {
    OptionHandler* op(new LocalFilePathOptionHandler(
        PREF_NETRC_PATH, TEXT_NETRC_PATH, util::getNetrc(),
        /* acceptStdin = */ false, 0, /* mustExist = */ false));
    handlers.push_back(op);
  }
  // Proxy options
