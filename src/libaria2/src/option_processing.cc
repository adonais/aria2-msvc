    ...
    if (!noConf) {
      ...
      if (File(cfname).isFile()) {
        std::stringstream ss;
        {
          ...
          if (fp) {
            A2_LOG_NOTICE(fmt("Loading conf \"%s\"", cfname.c_str()));
            fp.transfer(ss);
          }
        }
        try {
...
#  pragma GCC diagnostic pop
  ...
  if (op.getAsBool(PREF_ENABLE_NGWEB) && !op.getAsBool(PREF_ENABLE_RPC)) {
    A2_LOG_WARN("--enable-ngweb is disabled because of the rpc server not enabled");
    op.remove(PREF_ENABLE_NGWEB);
  }
