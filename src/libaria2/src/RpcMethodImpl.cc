...
namespace {
static void removeDownloadFile(const std::string &path) {
  // 排除根目录与相对目录, 排除可能存在的安全隐患
#ifdef _WIN32
  bool nl = path.length() > 3;
#else
  bool nl = path.length() > 1;
#endif
  if (nl && path[0] != '.' && path[0] != '[' && path.find("..") == std::string::npos) {
    File f(path);
    File f2(path + ".aria2");
    f.erase();
    f2.erase();
...
std::unique_ptr<ValueBase>
PurgeLocalFileRpcMethod::process(const RpcRequest& req, DownloadEngine* e)
{
  std::shared_ptr<DownloadResult> result = nullptr;
  const String* gidParam = checkRequiredParam<String>(req, 0);
  a2_gid_t gid = str2Gid(gidParam);
  auto group = e->getRequestGroupMan()->findGroup(gid);
  if (group) {
    ...
    if (!dctx) {
      throw DL_ABORT_EX(
          fmt("Local file requested for broken GID#%s",
              GroupId::toHex(gid).c_str()));
    }
    ...
    if (!path.empty()) {
      if (path.length() > 3 && path[0] != '[') {
        A2_LOG_NOTICE(fmt("GID#%s: Path [%s] will be deleted", GroupId::toHex(gid).c_str(), path.c_str()));  
      }
  ...
  else if ((result = e->getRequestGroupMan()->findDownloadResult(gid))) {
    std::shared_ptr<FileEntry> file = result->fileEntries[0];
    if (file && !result->dir.empty()) {
      int n = 0, m = 0;
      const std::string sep = File::getPathSeparators();
      std::string firstFilename = file->getPath();
    #ifdef _WIN32
      std::string first1 = toForwardSlash(firstFilename);
      std::string first2 = toForwardSlash(result->dir);
    #else
      std::string first1 = firstFilename;
      std::string first2 = result->dir;
    #endif
      std::string download = first1;
      n = static_cast<int>(download.length());
      while (((m = n) < 1024) && (n = download.find_last_of(sep)) != std::string::npos) {
        download = download.substr(0, n);
      #ifdef _WIN32
        if (stricmp(download.c_str(), first2.c_str()) == 0) {
      #else
        if (strcasecmp(download.c_str(), first2.c_str()) == 0) {
      #endif
            break;
        }
      }
      download = first1.substr(0, m);
      if (download.length() > 3 && download[0] != '[') {
        A2_LOG_NOTICE(fmt("GID#%s: Path [%s] will be deleted", GroupId::toHex(gid).c_str(), download.c_str()));
      }
      removeDownloadFile(download);
      e->getRequestGroupMan()->removeDownloadResult(gid);
    }
  }
  else {
    throw DL_ABORT_EX(
        fmt("Cannot get local file for GID#%s", GroupId::toHex(gid).c_str()));
  }
  return createOKResponse();
}
                                                         ...
                                                         DownloadEngine* e)
{
  ...
  std::string savename = util::createConvert(filename);
  if (sessionSerializer.save(savename)) {
    A2_LOG_NOTICE(
        fmt(_("Serialized session to '%s' successfully."), savename.c_str()));
    return createOKResponse();
  }
  throw DL_ABORT_EX(
      fmt("Failed to serialize session to '%s'.", savename.c_str()));
}
