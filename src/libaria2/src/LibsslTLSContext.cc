...
{
  std::string cert = util::createConvert(certfile);
  if (SSL_CTX_load_verify_locations(sslCtx_, cert.c_str(), nullptr) != 1) {
    A2_LOG_INFO(fmt(MSG_LOADING_TRUSTED_CA_CERT_FAILED, cert.c_str(),
                     ERR_error_string(ERR_get_error(), nullptr)));
    return false;
  }
  else {
    A2_LOG_NOTICE(fmt("Trusted CA certificates [%s] were successfully added.", cert.c_str()));
    return true;
  }
}
